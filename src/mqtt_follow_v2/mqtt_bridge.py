...
class MQTTBridge:
    ...
    def server_message_callback(self, topic, message):
        ...
        try:
            ...
            # 定义特殊指令（十六进制）
            START_CMD_HEX = "01050002ff002dfa"  # 开始巡线指令
            STOP_CMD_HEX = "01050002000028ca"   # 停止巡线指令（预留，可根据需要修改）
            ...
            if hex_representation == START_CMD_HEX:
                rospy.loginfo("[MQTT Bridge] 收到开始巡线指令 (0x%s)", START_CMD_HEX.upper())
            ...
            if byte_data is not None and len(byte_data) > 0:
                ...
                rospy.loginfo("[MQTT Bridge] 字节指令已发送到继电器: %s", hex_representation)
                return
