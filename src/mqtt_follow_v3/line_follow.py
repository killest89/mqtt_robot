...
class LineFollower:
    def __init__(self):
        ...
        # 状态机初始化
        self.state = "WAITING"  # WAITING, FOLLOWING, STOPPED, AVOIDING, RECHARGING, MANUAL_FOLLOWING
        self.stop_start_time = 0
        self.avoid_start_time = 0
        self.follow_start_time = time.time()
        self.waiting_start_time = time.time()
        self.recharge_start_time = 0
        self.manual_follow_start_time = 0
    ...
    def control_callback(self, msg):
        ...
        if command == "start" and (self.state == "WAITING" or self.state == "RECHARGING"):
            ...
            # 显示静态的WAITING状态窗口
            self.show_waiting_status()
            rospy.loginfo("[Line Follower] 手动模式停止寻线任务")
        ...
        try:
            ...
            if self.state == "FOLLOWING":
                ...
                if not red_strong and self.detect_green_segment(hsv):
                    rospy.loginfo("[Line Follower] [自动模式] 检测到绿色线段，切换到RECHARGING状态")
                    ...
                    rospy.loginfo("[Line Follower] [自动模式] 检测到黄色线段，切换到STOPPED状态")
            ...
            elif self.state == "MANUAL_FOLLOWING":
                # 手动模式：只跟红线 + 检测绿线，忽略黄线
                red_strong = hasattr(self, 'red_ratio') and self.red_ratio is not None and self.red_ratio >= 0.15
                # 检测绿线 - 绿线逻辑与自动模式一致
                if not red_strong and self.detect_green_segment(hsv):
                    rospy.loginfo("[Line Follower] [手动模式] 检测到绿色线段，切换到RECHARGING状态")
                    self.state = "RECHARGING"
                    self.recharge_start_time = time.time()
                    ...
                    end_msg.data = "follow_end"
                    self.relay_trigger_pub.publish(end_msg)
                    rospy.loginfo("[Line Follower] [手动模式] 已发送 follow_end 消息，等待 mqtt_bridge 处理回充")
            ...
            # 线段跟踪逻辑
            if self.state == "FOLLOWING":
                self.follow_line(image, hsv, allow_yellow_fallback=False)
            elif self.state == "MANUAL_FOLLOWING":
                self.follow_line(image, hsv, allow_yellow_fallback=True)
    ...
    def follow_line(self, image, hsv, allow_yellow_fallback=False):
        """红线跟踪：双区间红掩码 + 开闭形态学 + 自适应ROI
        当 allow_yellow_fallback=True（手动模式）时，红线丢失会尝试跟踪黄线"""
        ...
        if M['m00'] > 0:
            ...
            red_pixels = cv2.countNonZero(red_mask[search_top:search_bot, :])
            ...
            yellow_found = False
            if allow_yellow_fallback:
                # 手动模式下，红线丢失时尝试跟踪黄线
                lower_yellow = np.array([col_yellow[0], col_yellow[1], col_yellow[2]], dtype=np.uint8)
                upper_yellow = np.array([col_yellow[3], col_yellow[4], col_yellow[5]], dtype=np.uint8)
                ...
                if M_yellow['m00'] > 0:
                    ...
                    self.red_miss_consec = 0  # 黄线找到也重置miss计数

                    cx = int(M_yellow['m10'] / M_yellow['m00'])
                    cy = int(M_yellow['m01'] / M_yellow['m00'])
                    cv2.circle(image, (cx, cy), 10, (0, 255, 255), -1)  # 黄色标记点
                    ...
                    self.last_error = error
                    rospy.loginfo_throttle(2, "[Line Follower] [手动模式] 红线丢失，正在跟踪黄线")

            if not yellow_found:
                self.twist.linear.x = 0
                self.twist.angular.z = 0.2  # 小幅度旋转寻找线段

        if self.show_debug_window:
            # 红线跟踪时黄色框，黄线跟踪时蓝色框
            roi_color = (255, 100, 0) if tracking_color == "yellow" else (0, 255, 255)
            cv2.rectangle(image, (0, search_top), (w, search_bot), roi_color, 2)
    ...
    def display_image(self, image):
        ...
        try:
            ...
            state_duration = time.time() - {
                ...
                "MANUAL_FOLLOWING": self.manual_follow_start_time,
